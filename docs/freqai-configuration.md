...
### AutoGluon time series parameters

The ``AutoGluonTimeSeriesPredictor`` wraps AutoGluon's ``TimeSeriesPredictor``.
It accepts all parameters of ``TimeSeriesPredictor.fit`` and additionally
uses ``freqai.model_training_parameters`` to configure the data frequency via
the ``freq`` field.  ``prediction_length`` defaults to
``feature_parameters.label_period_candles`` if not provided.
