...
| `feature_importance_threshold` | Drop features with AutoGluon's feature importance below this value and retrain the model without them. Removing unimportant features can significantly speed up both training and prediction. <br> **Datatype:** Float. <br> Default: `0` (disabled). |
